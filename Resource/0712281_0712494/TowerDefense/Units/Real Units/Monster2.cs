...
{
    public class Monster2 : GroundCreep
    {
        ...
        public static void Initialize(XmlNodeList xmlNodeList)
        {
            ...
            foreach (XmlNode xmlNode in xmlNodeList)
            {
                switch (xmlNode.Name)
                {
                    ...
                    case "IntervalTime":
                        {
                            Monster2.iIntervalTime = int.Parse(xmlNode.InnerText);
                            break;
                        }
                    case "MaxLife":
                        {
                            Monster2.iBaseMaxLife = int.Parse(xmlNode.InnerText);
                            break;
                        }
                    case "MaxEnergy":
                        {
                            Monster2.iBaseMaxEnergy = int.Parse(xmlNode.InnerText);
                            break;
                        }
                    case "Speed":
                        {
                            Monster2.iBaseSpeed = int.Parse(xmlNode.InnerText);
                            break;
                        }
                    case "Defense":
                        {
                            Monster2.iBaseDefense = int.Parse(xmlNode.InnerText);
                            break;
                        }
                }
            }
        ...
        public Monster2(Vector2 vt2Position)
        {
            // gán giá trị static (nhập vào từ file) cho object mới
            this._vt2Position = vt2Position;

            this._iMaxLife = Monster2.iBaseMaxLife;
            this._iMaxEnergy = Monster2.iBaseMaxEnergy;
            this._iBaseSpeed = Monster2.iBaseSpeed;
            this._iBaseDefense = Monster2.iBaseDefense;
            this._iBaseIntervalTime = Monster2.iIntervalTime;
            ...
            this._iHeight = Monster2.iHeight;
            this._iWidth = Monster2.iWidth;
            this._effect = null;
            this._fDepth = 1.0f;
        ...
        public override Unit Clone(Vector2 vt2Position)
        {
            ...
            return new Monster2(vt2Position);
        }
