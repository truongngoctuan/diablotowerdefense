...
{
    public class Monster3 : GroundCreep
    {
        ...
        public static void Initialize(XmlNodeList xmlNodeList)
        {
            ...
            foreach (XmlNode xmlNode in xmlNodeList)
            {
                switch (xmlNode.Name)
                {
                    ...
                    case "IntervalTime":
                        {
                            Monster3.iIntervalTime = int.Parse(xmlNode.InnerText);
                            break;
                        }
                    case "MaxLife":
                        {
                            Monster3.iBaseMaxLife = int.Parse(xmlNode.InnerText);
                            break;
                        }
                    case "MaxEnergy":
                        {
                            Monster3.iBaseMaxEnergy = int.Parse(xmlNode.InnerText);
                            break;
                        }
                    case "Speed":
                        {
                            Monster3.iBaseSpeed = int.Parse(xmlNode.InnerText);
                            break;
                        }
                    case "Defense":
                        {
                            Monster3.iBaseDefense = int.Parse(xmlNode.InnerText);
                            break;
                        }
                }
            }
        ...
        public Monster3(Vector2 vt2Position)
        {
            // gán giá trị static (nhập vào từ file) cho object mới
            this._vt2Position = vt2Position;

            this._iMaxLife = Monster3.iBaseMaxLife;
            this._iMaxEnergy = Monster3.iBaseMaxEnergy;
            this._iBaseSpeed = Monster3.iBaseSpeed;
            this._iBaseDefense = Monster3.iBaseDefense;
            this._iBaseIntervalTime = Monster3.iIntervalTime;
            ...
            this._iHeight = Monster3.iHeight;
            this._iWidth = Monster3.iWidth;
            this._effect = null;
            this._fDepth = 1.0f;
        ...
        public override Unit Clone(Vector2 vt2Position)
        {
            ...
            return new Monster3(vt2Position);
        }
