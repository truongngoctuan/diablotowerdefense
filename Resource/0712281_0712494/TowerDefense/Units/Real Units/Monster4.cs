...
{
    public class Monster4 : GroundCreep
    {
        ...
        public static void Initialize(XmlNodeList xmlNodeList)
        {
            ...
            foreach (XmlNode xmlNode in xmlNodeList)
            {
                switch (xmlNode.Name)
                {
                    ...
                    case "IntervalTime":
                        {
                            Monster4.iIntervalTime = int.Parse(xmlNode.InnerText);
                            break;
                        }
                    case "MaxLife":
                        {
                            Monster4.iBaseMaxLife = int.Parse(xmlNode.InnerText);
                            break;
                        }
                    case "MaxEnergy":
                        {
                            Monster4.iBaseMaxEnergy = int.Parse(xmlNode.InnerText);
                            break;
                        }
                    case "Speed":
                        {
                            Monster4.iBaseSpeed = int.Parse(xmlNode.InnerText);
                            break;
                        }
                    case "Defense":
                        {
                            Monster4.iBaseDefense = int.Parse(xmlNode.InnerText);
                            break;
                        }
                }
            }
        ...
        public Monster4(Vector2 vt2Position)
        {
            // gán giá trị static (nhập vào từ file) cho object mới
            this._vt2Position = vt2Position;

            this._iMaxLife = Monster4.iBaseMaxLife;
            this._iMaxEnergy = Monster4.iBaseMaxEnergy;
            this._iBaseSpeed = Monster4.iBaseSpeed;
            this._iBaseDefense = Monster4.iBaseDefense;
            this._iBaseIntervalTime = Monster4.iIntervalTime;
            ...
            this._iHeight = Monster4.iHeight;
            this._iWidth = Monster4.iWidth;
            this._effect = null;
            this._fDepth = 1.0f;
        ...
        public override Unit Clone(Vector2 vt2Position)
        {
            ...
            return new Monster4(vt2Position);
        }
