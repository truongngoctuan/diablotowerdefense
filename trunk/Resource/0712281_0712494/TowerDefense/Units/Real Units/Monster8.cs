...
{
    public class Monster8 : GroundCreep
    {
        ...
        public static void Initialize(XmlNodeList xmlNodeList)
        {
            ...
            foreach (XmlNode xmlNode in xmlNodeList)
            {
                switch (xmlNode.Name)
                {
                    ...
                    case "IntervalTime":
                        {
                            Monster8.iIntervalTime = int.Parse(xmlNode.InnerText);
                            break;
                        }
                    case "MaxLife":
                        {
                            Monster8.iBaseMaxLife = int.Parse(xmlNode.InnerText);
                            break;
                        }
                    case "MaxEnergy":
                        {
                            Monster8.iBaseMaxEnergy = int.Parse(xmlNode.InnerText);
                            break;
                        }
                    case "Speed":
                        {
                            Monster8.iBaseSpeed = int.Parse(xmlNode.InnerText);
                            break;
                        }
                    case "Defense":
                        {
                            Monster8.iBaseDefense = int.Parse(xmlNode.InnerText);
                            break;
                        }
                }
            }
        ...
        public Monster8(Vector2 vt2Position)
        {
            // gán giá trị static (nhập vào từ file) cho object mới
            this._vt2Position = vt2Position;

            this._iMaxLife = Monster8.iBaseMaxLife;
            this._iMaxEnergy = Monster8.iBaseMaxEnergy;
            this._iBaseSpeed = Monster8.iBaseSpeed;
            this._iBaseDefense = Monster8.iBaseDefense;
            this._iBaseIntervalTime = Monster8.iIntervalTime;
            ...
            this._iHeight = Monster8.iHeight;
            this._iWidth = Monster8.iWidth;
            this._effect = null;
            this._fDepth = 1.0f;
        ...
        public override Unit Clone(Vector2 vt2Position)
        {
            ...
            return new Monster8(vt2Position);
        }
