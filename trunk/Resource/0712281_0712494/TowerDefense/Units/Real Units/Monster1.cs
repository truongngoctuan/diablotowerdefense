...
{
    public class Monster1 : GroundCreep
    {
        ...
        public static void Initialize(XmlNodeList xmlNodeList)
        {
            ...
            foreach (XmlNode xmlNode in xmlNodeList)
            {
                switch (xmlNode.Name)
                {
                    ...
                    case "IntervalTime":
                        {
                            Monster1.iIntervalTime = int.Parse(xmlNode.InnerText);
                            break;
                        }
                    case "MaxLife":
                        {
                            Monster1.iBaseMaxLife = int.Parse(xmlNode.InnerText);
                            break;
                        }
                    case "MaxEnergy":
                        {
                            Monster1.iBaseMaxEnergy = int.Parse(xmlNode.InnerText);
                            break;
                        }
                    case "Speed":
                        {
                            Monster1.iBaseSpeed = int.Parse(xmlNode.InnerText);
                            break;
                        }
                    case "Defense":
                        {
                            Monster1.iBaseDefense = int.Parse(xmlNode.InnerText);
                            break;
                        }
                }
            }
        ...
        public Monster1(Vector2 vt2Position)
        {
            // gán giá trị static (nhập vào từ file) cho object mới
            this._vt2Position = vt2Position;

            this._iMaxLife = Monster1.iBaseMaxLife;
            this._iMaxEnergy = Monster1.iBaseMaxEnergy;
            this._iBaseSpeed = Monster1.iBaseSpeed;
            this._iBaseDefense = Monster1.iBaseDefense;
            this._iBaseIntervalTime = Monster1.iIntervalTime;
            ...
            this._iHeight = Monster1.iHeight;
            this._iWidth = Monster1.iWidth;
            this._effect = null;
            this._fDepth = 1.0f;
        ...
        public override Unit Clone(Vector2 vt2Position)
        {
            ...
            return new Monster1(vt2Position);
        }
